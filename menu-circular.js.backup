...
//el nombre se usa como título para el bubblene
//el icono es la identificación del símbolo svg correspondiente
const services_data = [
...
//Mantiene el código DRY evitando el espacio de nombres para la creación de elementos
function createSVGElement(el){
  return document.createElementNS("http://www.w3.org/2000/svg", el)
}

//Configuración rápida para múltiples atributos
...
//Las burbujas de servicio se crean dinámicamente
function addService(serv,index) {
  let group = createSVGElement("g");
  group.setAttribute("class", "service serv-"+index);

  /* Este grupo es necesario para aplicar animaciones en
    el icono y su fondo al mismo tiempo */
  ...
  //Utiliza interpolación para buscar la posición correcta
...
//Define el comportamiento de las burbujas de servicio
function serviceClick(ev){
  
  //Siempre hay un servicio activo
